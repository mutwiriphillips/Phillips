        ...
        {citizenCategories.map((c) => (
          <Card key={c.title}>
            ...
            <ul className="space-y-2">
              {c.items.map((it) => {
                const inCart = has("citizenease", it.name);
                return (
                  <li key={it.name} className="text-sm">
                    <div className="flex items-start gap-1.5 text-[#6B6153] dark:text-[#9AA3B5]">
                      <ChevronRight size={13} className="mt-0.5 flex-shrink-0 text-clay" />
                      <span className="flex-1">{it.name}</span>
                    </div>
                    <button
                      onClick={() => add({ division: "citizenease", name: it.name, docs: it.docs })}
                      disabled={inCart}
                      className="mt-1 ml-[19px] text-xs font-semibold inline-flex items-center gap-1 text-clay disabled:opacity-50 disabled:cursor-default"
                    >
                      {inCart ? (
                        <>
